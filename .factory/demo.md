...
The demo writes only the `demo:code-listen-cursor:pronunciation` local-storage key, and only after a sample pronunciation is saved. **Reset demo** removes every `demo:` key and restores the shipped code and controls. **Start for real** removes every `demo:` key, then opens the installation section on `/`. It never reads or writes real extension data.
